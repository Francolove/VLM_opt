...
!            write(900,*) u4+U
            ...
            !CALL cross(U+u2, x2, f2)
            !CALL cross(U+u3, x3, f3);
            CALL cross(U+u4, x4, f4)

            ! controllo su f1
            IF (i == 1 .OR. MOD(i,n+1)==0) THEN ! sono sul pannello estremo a sinistra/destra
               f1 = -f1 * Vortices(i)%ptrVortex%Gamma
            ELSEIF (MOD(i,n)/=0) THEN
               f1 = f1 * (-Vortices(i)%ptrVortex%Gamma + Vortices(i-1)%ptrVortex%Gamma)
            ELSE !ultimo pannello in apertura
               ...
               f1 = f1 * (-Vortices(i)%ptrVortex%Gamma + Vortices(i-1)%ptrVortex%Gamma) + &
                    f3* Vortices(i)%ptrVortex%Gamma
            ENDIF


            ! controllo su f4
            IF (i <= n) THEN ! sono sul pannello estremo a sinistra/destra
               f4 = f4 * Vortices(i)%ptrVortex%Gamma
            ELSE
               f4 = f4 * (-Vortices(i-n)%ptrVortex%Gamma + Vortices(i)%ptrVortex%Gamma)
            ENDIF
            f = f + f1 + f4
